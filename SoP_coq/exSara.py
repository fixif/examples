...
R1 = State_Space(Filter(A=A, B=B.transpose(), C=C, D=D, name='longitudinal'))   # 'longitudinal oscillation controller DL'
...
A, B, C, D = [mat(d[x]) for x in ('A', 'B', 'C', 'D')]
R2 = State_Space(Filter(A=A, B=B.transpose(), C=C, D=D, name='SDR'))            # Software-Defined Radio
...
R4 = State_Space(Filter(A=A,B=B,C=C,D=D,name='ISCAS2006'))
...
[                0                  0   0.00123382999457  -0.00072504533047  -0.00222099952949;
                 ...
                 0                  0  -0.00072504533047   0.00043303317469   0.00152133486438;
                 0                  0  -0.00222099952949   0.00152133486438   0.01130345144690 ]""")
R5 = State_Space(Filter(A=A,B=B,C=C,D=D,name='exPC'))


lS = [State_Space(Filter(ss=R5.dSS[i, :])) for i in range(7)]
lS.append(R5)
...
for i,R in enumerate([R1,R2,R3,R4,R5,R6]):
	with open('Sara/R%d.c'%i,'w') as f:
		f.write(R.implementCdouble(R.shortName))
	with open('Sara/R%d.algo.txt'%i,'w') as f:
		f.write(R.algorithmLaTeX(R.shortName),'%.6f')